    ...
    def wrapper(self, *args, **kwargs):
        if func.__name__ == "_sample_points":
            ...
            if len(self.fitting_times) < self.n_evals - 1:
                self.fitting_times.extend(
                    [0] * (self.n_evals - len(self.fitting_times))
                )
            self.fitting_times.append(end_time - start_time)
            return gp
        elif (
            func.__name__ == "_select_next_point"
            or func.__name__ == "_select_next_points"
            or func.__name__ == "_local_search"
        ):
            start_time = time.perf_counter()
            next_points = func(self, *args, **kwargs)
            end_time = time.perf_counter()
            if len(self.selecting_times) < self.n_evals:
                self.selecting_times.extend(
                    [0] * (self.n_evals - len(self.selecting_times))
                )
            ...
            if self.opt_start_time is not None:
                ...
                self.optimizing_times.extend(
                    [opt_time] * (self.n_evals - len(self.optimizing_times))
                )
...
class BayesmarkExpRunner:
    def __init__(self, task_context, dataset, seed, use_log=False):
        ...
        for hyperparam in self.ordered_hyperparams:
            ...
            search_lower_bounds.append(
                self._space_type_to_linear(_space_type, constraint[2][0])
            )
            search_upper_bounds.append(
                self._space_type_to_linear(_space_type, constraint[2][1])
            )
    ...
    def _map_bounds(self, x, bounds, new_bounds):
        ...
        """
        scaled_x = new_bounds[0] + (x - bounds[0]) * (new_bounds[1] - new_bounds[0]) / (
            bounds[1] - bounds[0]
        )
        return scaled_x
    ...
    def generate_initialization(self, n_samples):
        ...
        init_configs = pd.read_json(
            f"Benchmarks/LLAMBO/bayesmark/configs/{self.model}/{self.seed}.json"
        ).head(n_samples)
...
def _run_bayesmark_exp(
    ...
    if (
        num_seeds is not None
        and isinstance(num_seeds, int)
        and (num_seeds < 1 or num_seeds > 10)
    ):
    ...
    for seed in range(num_seeds):
        logger.info(
            "Executing %s to tune %s on %s with seed %d / %d and use_log %d...",
            algo,
            model,
            dataset,
            seed + 1,
        ...
        if is_profile:
            ...
            df_times.to_csv(
                f"{_save_res_dir}/{algo}_{model}_{dataset}_{seed}.csv", index=False
            )
...
def plot_bayesmark_results():
            ...
            for _model in os.listdir(os.path.join(dir_path, _dataset)):
                for _res_file in os.listdir(os.path.join(dir_path, _dataset, _model)):
                    if _res_file.endswith(".csv"):
                        ...
                        df = pd.read_csv(
                            os.path.join(dir_path, _dataset, _model, _res_file)
                        )
                        dim = df.shape[1] - 2
                        ...
                        if _model not in data_map:
                            ...
                            data_map[_model] = pd.concat(
                                [data_map[_model], df], ignore_index=True
                            )
    ...
    for _model, df in data_map.items():
        ...
        best_df = df[
            ["algo", "short_algo", "dataset", "model", "seed", "generalization_score"]
        ]
        best_df = (
            best_df.groupby(["algo", "short_algo", "dataset", "model", "seed"])
            .agg(list)
            .reset_index()
        )
        ...
        mean_best_df = (
            best_df.groupby(["algo", "short_algo", "dataset", "model"])
            .agg(np.mean)
            .reset_index()
        )
        mean_best_df.drop(columns=["seed"], inplace=True)
        mean_best_df.to_csv(
            os.path.join(csv_save_dir, f"{_model}_best.csv"), index=False
        )
        ...
        for dataset in datasets:
            ...
            for algo in algos:
                ...
                best_y_data = (
                    np.maximum.accumulate(y_data, axis=1)
                    if is_maximization
                    else np.minimum.accumulate(y_data, axis=1)
                )
                ...
                _y_regret = np.abs(y_data - _regret_min) / np.abs(
                    _regret_max - _regret_min
                )
                _best_y_regret = np.minimum.accumulate(_y_regret, axis=1)
                _mean_y_regret = np.mean(_best_y_regret, axis=0)
                _algo_regret_filling.append(
                    [np.min(_best_y_regret, axis=0), np.max(_best_y_regret, axis=0)]
                )
            ...
            plot_filling.append(
                [[_l[clip_index:], _r[clip_index:]] for _l, _r in _algo_filling]
            )

            plot_y_regret.append([_y[clip_index:] for _y in _algo_regret])
            plot_filling_regret.append(
                [[_l[clip_index:], _r[clip_index:]] for _l, _r in _algo_regret_filling]
            )
    ...
    overall_regret_df = (
        regret_df.groupby(["algo", "short_algo", "dataset_type", "seed"])[
            ["best_regret"]
        ]
        .agg(_axis0_mean)
        .reset_index()
    ...
    for dataset_type in dataset_types:
        overall_type_df = overall_regret_df[
            overall_regret_df["dataset_type"] == dataset_type
        ]
        ...
        type_plot_filling.append(
            [[_l[clip_index:], _r[clip_index:]] for _l, _r in _type_filling]
        )
    ...
    overall_regret_df = (
        regret_df.groupby(["algo", "model", "dataset_type", "seed"])[["best_regret"]]
        .agg(_axis0_mean)
        .reset_index()
    )
    ...
    for i, data_type in enumerate(data_types):
        for j, model in enumerate(models):
            ...
            model_plot_filling.append(
                [[_l[clip_index:], _r[clip_index:]] for _l, _r in _model_filling]
            )
    ...
    overall_model_metrics_df = (
        regret_df.groupby(["algo", "model", "seed", "metric"])[["best_g_score"]]
        .agg(_axis0_mean)
        .reset_index()
    )
    y_label_datasets = set(["breast", "diabetes"])
    ...
    for m_i, metric in enumerate(metrics):
        _metric_df = overall_model_metrics_df[
            overall_model_metrics_df["metric"] == metric
        ]
    ...
    overall_dataset_df = (
        regret_df.groupby(["algo", "dataset", "seed"])[["best_g_score"]]
        .agg(_axis0_mean)
        .reset_index()
    )
    datasets = [
    ...
    for dataset in datasets:
        ...
        dataset_plot_filling.append(
            [[_l[clip_index:], _r[clip_index:]] for _l, _r in _dataset_filling]
        )
...
def plot_bayesmark_profile_results():
    dir_paths = ["Benchmarks/bayesmark_profile_results"]
    ...
    for model in models:
        for time_type in time_types:
            ...
            for _algo in algos:
                ...
                df = algo_map[_algo]
                time_df = (
                    df.groupby(["model", "dataset", "algo", "seed"])
                    .agg(list)
                    .reset_index()
                )
                ...
                for _res_file in os.listdir(os.path.join(dir_path, _dataset, _model)):
                    if _res_file.endswith(".csv"):
                        ...
                        if _model not in df_data:
                            ...
                            df_data[_model] = pd.concat(
                                [df_data[_model], df], ignore_index=True
                            )
...
if __name__ == "__main__":
    ...
    from Experiments.generated_algorithms.AdaptiveTrustRegionEvolutionaryBO_DKAB_aDE_GE_VAE import (
        AdaptiveTrustRegionEvolutionaryBO_DKAB_aDE_GE_VAE,
    )
    from Experiments.generated_algorithms.AdaptiveTrustRegionOptimisticHybridBO import (
        AdaptiveTrustRegionOptimisticHybridBO,
    )
    from Experiments.generated_algorithms.AdaptiveEvolutionaryParetoTrustRegionBO import (
    ...
    from Experiments.baselines.bo_baseline import (
        BLVanillaEIBO,
        BLCMAES,
        BLHEBO,
        BLTuRBO1,
    ...
    is_profile = False
    bo_wrappers = [
        bayesmarkBO_wrapper(bo_cls, is_profile=is_profile) for bo_cls in bo_cls_list
    ]
