...
class HPOExpRunner:
    def __init__(self, task_context, dataset, seed):
        ...
        self.seed = seed
        self.config_path = (
            f"Benchmarks/LLAMBO/hpo_bench/configs/{model}/config{seed}.json"
        )
    ...
    def _find_nearest_neighbor(self, config):
        ...
        for key in config:
            if key in discrete_grid:
                ...
                nearest_value = min(
                    discrete_grid[key], key=lambda x: abs(x - config[key])
                )
...
def download_dataset():
    ...
    for name, url in urls.items():
        ...
        curl_command = ["curl", "-L", "-#", "-o", str(temp_zip_path), url]
...
def run_hpo_benchmarks(
    bo_cls, dataset_name, model, seeds_to_run, n_initial_samples=5, budget=30
):
    ...
    X, y, categorical_mask, _ = dataset_.get_data(
        target=dataset_.default_target_attribute
    )
    ...
    for seed in seeds_to_run:
        logger.info(
            "Executing %s to tune %s on %s with seed %d / %d...",
            algo,
            model,
            dataset_name,
            seed + 1,
...
if __name__ == "__main__":
    ...
    from Experiments.logs.algorithms_logs.AdaptiveTrustRegionEvolutionaryBO_DKAB_aDE_GE_VAE import (
        AdaptiveTrustRegionEvolutionaryBO_DKAB_aDE_GE_VAE,
    )
    from Experiments.logs.algorithms_logs.AdaptiveTrustRegionOptimisticHybridBO import (
        AdaptiveTrustRegionOptimisticHybridBO,
    )
    from Experiments.logs.algorithms_logs.AdaptiveEvolutionaryParetoTrustRegionBO import (
