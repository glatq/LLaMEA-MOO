...
class MOBO_MOEAD_EI_Hybrid_Fixed:
    ...
    def __init__(self, budget: int, dim: int, bounds: np.ndarray | None = None,
                 ...
                 gp_n_restarts_optimizer: int = 1): # Number of restarts for GP hyperparameter optimization
        ...
        self.X_pop: np.ndarray | None = None # Current population inputs (true evaluated)
        self.F_pop: np.ndarray | None = None # Current population objectives (true evaluated)
        ...
        self.n_init = max(min(int(self.budget * self.n_init_ratio), 2 * self.dim + 1), 1)
        self.n_init = min(self.n_init, self.budget) # Ensure n_init doesn't exceed budget
        ...
        self.ideal_point: np.ndarray | None = None # Dynamically updated ideal point (min observed)
        self.nadir_point: np.ndarray | None = None # Dynamically updated nadir point (max observed)
    ...
    def _generate_weight_vectors(self, n_obj: int, population_size: int) -> np.ndarray:
        ...
        """
        if n_obj == 2:
            if population_size < 2: return np.array([[0.5, 0.5]]) # Handle edge case
            weights = np.array([[i / (population_size - 1), (population_size - 1 - i) / (population_size - 1)]
    ...
    def _augmented_tchebycheff_scalarization(self, objectives: np.ndarray, weights: np.ndarray, ideal_point: np.ndarray, rho: float) -> np.ndarray:
        """
    ...
    def _polynomial_mutation(self, x: np.ndarray, lb: np.ndarray, ub: np.ndarray, eta_m: float) -> np.ndarray:
        """
        ...
        if np.any(mutate_mask):
            delta1 = (y[mutate_mask] - lb[mutate_mask]) / (ub[mutate_mask] - lb[mutate_mask])
            delta2 = (ub[mutate_mask] - y[mutate_mask]) / (ub[mutate_mask] - lb[mutate_mask])
    ...
    def _sbx_crossover(self, parent1: np.ndarray, parent2: np.ndarray, lb: np.ndarray, ub: np.ndarray, eta_c: float) -> tuple[np.ndarray, np.ndarray]:
        """
        ...
        if np.random.rand() < self.crossover_prob:
            ...
            beta[mask_gt_05] = (1.0 / (2.0 * (1.0 - rand_val[mask_gt_05]))) ** (1.0 / (eta_c + 1.0))
            
            # Apply SBX formulas
            offspring1_new = 0.5 * (((parent1 + parent2) - beta * np.abs(parent2 - parent1)))
            offspring2_new = 0.5 * (((parent1 + parent2) + beta * np.abs(parent2 - parent1)))
    ...
    def _sample_points(self, n_points: int, sampler_type: str = 'lhs') -> np.ndarray:
        ...
        if sampler_type == 'lhs':
            sampler = qmc.LatinHypercube(d=self.dim, seed=np.random.randint(0, 2**32-1))
    ...
    def _fit_model(self, X: np.ndarray, y: np.ndarray):
        ...
        for i in range(self.n_obj):
            ...
            kernel = ConstantKernel(1.0, (1e-3, 1e3)) * Matern(length_scale=[1.0]*self.dim, length_scale_bounds=(1e-2, 1e2), nu=self.kernel_nu) \
            ...
            gp = GaussianProcessRegressor(kernel=kernel,
                                          ...
                                          random_state=42 + i) # Different random state for each GP for diversity
            try:
                gp.fit(X_train_scaled, y_train[:, i])
            except ValueError: # Catch potential errors like singular matrices, retry with fewer restarts
                gp = GaussianProcessRegressor(kernel=Matern(length_scale=[1.0]*self.dim, nu=self.kernel_nu) + WhiteKernel(noise_level=1e-5),
                                              normalize_y=True,
    ...
    def _select_next_points(self, batch_size: int) -> np.ndarray:
        ...
        if self.X_pop is None or self.F_pop is None or self.X_all is None or self.y_all is None:
        ...
        for _ in range(self.n_offspring_surrogate // 2): # Generate pairs
            ...
            child1, child2 = self._sbx_crossover(parent1, parent2, self.bounds[0], self.bounds[1], self.eta_c)
            child1 = self._polynomial_mutation(child1, self.bounds[0], self.bounds[1], self.eta_m)
            child2 = self._polynomial_mutation(child2, self.bounds[0], self.bounds[1], self.eta_m)
            
            X_offspring_generated.extend([child1, child2])
        
        X_offspring_generated = np.array(X_offspring_generated[:self.n_offspring_surrogate])
        ...
        mu_offspring = np.array(mu_offspring_list).T # Shape (n_offspring_surrogate, n_obj)
        sigma_offspring = np.array(sigma_offspring_list).T # Shape (n_offspring_surrogate, n_obj)
        ...
        obj_range_full_archive[obj_range_full_archive == 0] = 1e-6 # Avoid division by zero
        
        # Normalize all past observed objectives (for f_min_scalar)
        y_all_normalized = (self.y_all - ideal_point_full_archive) / obj_range_full_archive

        # Normalize predicted objectives for offspring (for mu_s and sigma_s)
        mu_offspring_normalized = (mu_offspring - ideal_point_full_archive) / obj_range_full_archive
        sigma_offspring_normalized = sigma_offspring / obj_range_full_archive # Sigma also scaled by range
        ...
        for j in range(self.population_size):
            ...
            f_min_scalar_j = np.min(self._augmented_tchebycheff_scalarization(
            ...
            weighted_mu_terms = w_j * (mu_offspring_normalized - np.zeros(self.n_obj)) # (n_offspring_surrogate, n_obj)
            k_max_all = np.argmax(weighted_mu_terms, axis=1) # Index of the max objective for each candidate
            sigma_scalar_offspring = sigma_offspring_normalized[np.arange(X_offspring_generated.shape[0]), k_max_all]
            ...
            u[mask_positive_sigma] = (f_min_scalar_j - tcheby_mean_offspring[mask_positive_sigma]) / sigma_scalar_offspring[mask_positive_sigma]
            ei_vals_for_w_j[mask_positive_sigma] = sigma_scalar_offspring[mask_positive_sigma] * (u[mask_positive_sigma] * norm.cdf(u[mask_positive_sigma]) + norm.pdf(u[mask_positive_sigma]))
        ...
        best_offspring_indices_per_subproblem = np.argmax(ei_matrix, axis=0) # indices into X_offspring_generated
        ...
        if len(unique_selected_indices) > batch_size:
            max_ei_for_unique_candidates = np.max(ei_matrix[unique_selected_indices], axis=1)
            sorted_unique_indices = unique_selected_indices[np.argsort(max_ei_for_unique_candidates)[::-1]] # Sort descending
            ...
            if len(selected_X_indices) < batch_size:
                ...
                if len(available_indices) > 0:
                    additional_indices = np.random.choice(available_indices, min(remaining_slots, len(available_indices)), replace=False)
                    selected_X_indices = np.concatenate((selected_X_indices, additional_indices))
    ...
    def _evaluate_points(self, func: Callable[[np.ndarray], np.ndarray], X_to_consider: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        X_evaluated = X_to_consider[:num_points_to_eval] # This is the actual X being evaluated
        ...
        for x_point in X_clipped: # Iterate over X_clipped
            ...
            if y_val.shape[0] != self.n_obj:
                raise ValueError(f"Function returned {y_val.shape[0]} objectives, but expected {self.n_obj}.")
        ...
        return np.array(y_results), X_evaluated # Return the actual X that was evaluated
    ...
    def _update_eval_points(self, new_X: np.ndarray, new_F: np.ndarray):
        ...
        if self.X_pop is None:
            ...
            self.weights = self._generate_weight_vectors(self.n_obj, self.population_size)
            ...
            for x_offspring, f_offspring in zip(new_X, new_F):
                ...
                for subproblem_idx in range(self.population_size):
                    ...
                    tcheby_offspring = self._augmented_tchebycheff_scalarization(f_offspring_norm, w, np.zeros(self.n_obj), self.rho)
                    tcheby_current_pop = self._augmented_tchebycheff_scalarization(current_f_pop_norm, w, np.zeros(self.n_obj), self.rho)
    ...
    def __call__(self, func: Callable[[np.ndarray], np.ndarray]) -> tuple[np.ndarray, np.ndarray]:
        ...
        initial_y_evaluated, initial_X_evaluated = self._evaluate_points(func, initial_X_sampled)
        ...
        while self.n_evals < self.budget:
            ...
            next_y_evaluated, next_X_evaluated = self._evaluate_points(func, next_X_to_eval_raw)
