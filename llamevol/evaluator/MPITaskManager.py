...
warnings.filterwarnings(
    "ignore", message="the 'buf' argument is deprecated", category=UserWarning
)
...
class MPITaskManager(metaclass=Singleton):
    ...
    def _handle_interrupt(self, signum, frame):
        logger.debug(
            "Master received interrupt signal %s. Initiating shutdown...", signum
        )
    ...
    def submit(self, func, *args, **kwargs):
        ...
        if self.excuting:
            logger.error(
                "Task manager is already executing tasks, cannot submit new tasks"
            )
            return
        ...
        logger.debug(
            "Task %s added to queue. Queue length: %s", task_id, len(self.task_queue)
        )
    ...
    def wait(self, futures=None, timeout=None):
        if self.shutdown_requested or self.excuting:
            logger.error(
                "Task manager is shutting down or already executing tasks, cannot wait for futures"
            )
            return (set(), set())
    ...
    def shutdown(self, wait=True, cancel_futures=False, terminate_workers=False):
        ...
        logger.debug(
            "Shutdown requested with wait=%s, cancel_futures=%s, terminate_workers=%s",
            wait,
            cancel_futures,
            terminate_workers,
        )
        ...
        if wait and (self.task_queue or self.active_tasks):
            ...
            pending_futures.extend(
                [self.futures[task.task_id] for task in self.active_tasks.values()]
            )
        ...
        if terminate_workers:
            ...
            if wait:
                ...
                while any(
                    ...
                    for status in self.workers_status.values()
                ):
                    self._master_process_messages()
                    time.sleep(0.2)
        ...
        for worker_rank in range(1, self.size):
            if self.comm.Iprobe(source=worker_rank, tag=Tags.RESULT.value):
                ...
                try:
                    if self.result_recv_buffer is not None:
                        self.comm.recv(
                            self.result_recv_buffer,
                            source=worker_rank,
                            tag=Tags.RESULT.value,
                            status=status,
                        )
                        data_size = status.Get_count(MPI.BYTE)
                        result_package = MPI.pickle.loads(
                            self.result_recv_buffer[:data_size]
                        )
                    else:
                        result_package = self.comm.recv(
                            source=worker_rank, tag=Tags.RESULT.value, status=status
                        )
                        data_size = status.Get_count(MPI.BYTE)
                    logger.debug(
                        "Received result from worker %s with size %s",
                        worker_rank,
                        data_size,
                    )
            ...
            if self.comm.Iprobe(source=worker_rank, tag=Tags.STATUS.value):
                ...
                if status == Status.TERMINATED or status == Status.IDLE:
                    if worker_rank in self.active_tasks:
                        logger.error(
                            "Worker %s change status to %s while still running task %s",
                            worker_rank,
                            status,
                            self.active_tasks[worker_rank].task_id,
                        )
                        del self.active_tasks[worker_rank]
    ...
    def _assign_pending_tasks(self):
        idle_workers = [
            rank
            for rank, status in self.workers_status.items()
    ...
    def _execute_task(self, task, queue):
        ...
        try:
            ...
            logger.exception(
                "Worker %s encountered error on task %s", self.rank, task_id
            )
        ...
        while self.running_as_worker:
            ...
            if not current_task and self.comm.Iprobe(source=0, tag=Tags.TASK.value):
                ...
                if self.task_recv_buffer is not None:
                    ...
                    logger.debug(
                        "Worker %s received task %s with size %s from buffer",
                        self.rank,
                    ...
                    logger.debug(
                        "Worker %s received task %s", self.rank, current_task.task_id
                    )
                ...
                if self.sub_process_worker:
                    ...
                    res_queue = mp.Queue()
                    logger.debug(
                        "Worker %s starting sub process for task %s",
                        self.rank,
                        current_task.task_id,
                    )

                    process = mp.Process(
                        target=self._execute_task, args=(current_task, res_queue)
                    )
                    sub_process = process
                    process.start()
                else:
...
@contextmanager
def start_mpi_task_manager(
    task_recv_buffer_size=None,
    result_recv_buffer_size=None,
    ...
    if task_manager.is_master:
        if task_manager.shutdown_requested or task_manager.excuting:
            raise RuntimeError(
                "Task manager is already executing tasks or shutting down"
            )
