...
from .population import (
    ...
    PopulationQueryItem,
    desc_similarity,
    score_to_probability_with_logarithmic_curve,
)
...
class IslandESPopulation(Population):
    ...
    def __init__(
        ...
        for i in range(n_islands):
            wrapper = IslandESPopulation.IslandPopWrapper(
                ESPopulation(
                    n_parent, n_parent_per_offspring, n_offspring, use_elitism
                ),
                IslandESPopulation.IslandStatus.INITIAL,
            )
            wrapper.island_id = i
    ...
    def __is_the_end_of_age(self, generation: int, age: IslandAge):
        if age == self.IslandAge.WARMUP:
            ...
            return (generation - self.n_warmup_generations) % (
                self.n_cambrian_generations + self.n_neogene_generations
            ) == self.n_cambrian_generations - 1
        elif age == self.IslandAge.NEOGENE:
            return (generation - self.n_warmup_generations) % (
                self.n_cambrian_generations + self.n_neogene_generations
            ) == self.n_cambrian_generations + self.n_neogene_generations - 1
        return False
    ...
    def __get_all_mature_individuals(self, sort: bool = False, revserse: bool = False):
        ...
        if sort:
            all_mature_inds = sorted(
                all_mature_inds, key=lambda x: x.fitness, reverse=revserse
            )
    ...
    def __get_all_best_individuals(self, sort: bool = False, revserse: bool = False):
        ...
        if sort:
            all_best_inds = sorted(
                all_best_inds, key=lambda x: x.fitness, reverse=revserse
            )
    ...
    def __get_prob_migration_queryitems(
        self,
        all_best_inds: list[Individual],
        migration_batch: int,
        wrapper: IslandPopWrapper,
        ...
        migration_prob = score_to_probability_with_logarithmic_curve(
            score=my_best_ind.fitness, max_score=min(1.0, max_fitness * 1.5)
        )
        ...
        if np.random.random() > migration_prob:
            ...
            for p_item in p_items:
                ...
                query_item = PopulationQueryItem(
                    qid=wrapper.island_id, parent=parent, offspring=Individual()
                )
                items.append(query_item)
        return items

    def get_offspring_queryitems(
        self, n_parent: int = None
    ) -> list[PopulationQueryItem]:
        if self.preorder_aware_init and len(self.__preorder_init_queue) > 0:
        ...
        for wrapper in self.populations:
            ...
            if status == self.IslandStatus.INITIAL:
                ...
                items = wrapper.pop.get_offspring_queryitems(
                    n_parent=n_parent_per_offspring
                )
                ...
                if self.geo_age == self.IslandAge.CAMBRIAN:
                    if len(all_best_inds) > 1:
                        if self.cyclic_migration:
                            ...
                            query_items = self.__get_prob_migration_queryitems(
                                all_best_inds,
                ...
                for _ in range(wrapper.pop.n_offspring):
                    sample_size = min(
                        wrapper.pop.n_parent_per_offspring, len(all_best_inds)
                    )
                    parent = np.random.choice(
                        all_best_inds, size=sample_size, replace=False
                    ).tolist()
                    query_item = PopulationQueryItem(
                        qid=wrapper.island_id, parent=parent, offspring=Individual()
                    )
