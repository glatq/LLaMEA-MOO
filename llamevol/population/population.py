...
class Population(ABC):
    ...
    def __init__(self):
        self.name = None
        self.get_parent_strategy: Callable[
            [list[Individual], int, int], list[PopulationQueryItem]
        ] = None
        self.selection_strategy: Callable[
            [list[Individual], list[Individual], int], list[Individual]
        ] = None
    ...
    def get_next_queryitems(
        self, query_items: list[PopulationQueryItem]
    ) -> list[PopulationQueryItem]:
        return None
    ...
    def get_handler_from_individual(cls, individual: Individual):
        if individual is not None:
            return (
                individual.metadata["res_handler"]
                if "res_handler" in individual.metadata
                else None
            )
        return None
    ...
    def _update_save_dir_if_need(self):
        ...
        if self._save_dir_suffix is None:
            ...
            self._save_dir_suffix = (
                f"{self.__class__.__name__}_{file_name}_{time_stamp}"
            )

    def save_on_the_fly(
        self, individual: Individual, generation: int, index: int = None
    ):
    ...
    def save(self, filename=None, dirname=None, suffix=None):
        ...
        file_path = os.path.join(
            dirname, f"{self.__class__.__name__}{filename}_{time_stamp}.pkl"
        )
        try:
...
def code_diff_similarity(
    inds: list[Individual], max_workers=0
...
def code_diff_similarity_from_handlers(
    handlers: list, max_workers=0
) -> tuple[np.ndarray, np.ndarray]:
    if not handlers:
        return np.array([]), np.array([])
...
def _code_diff_similarity(codes: list, max_workers=0) -> tuple[np.ndarray, np.ndarray]:
    ...
    if max_workers > 0:
        with concurrent.futures.ProcessPoolExecutor(
            ...
            futures = {
                executor.submit(code_compare, *params[key]): key for key in params
            }
    ...
    mean_similarity = np.array(
        [
            np.mean(
                np.concatenate(
                    [similarity_matrix[i, :i], similarity_matrix[i, i + 1 :]]
                )
            )
...
def family_competition_selection_fn(parent_size_threshold=1, is_aggressive=True):
    def _family_selection(child, all_ind_map, parent_size_threshold):
        ...
        sorted_ids = sorted(
            [ind_id for ind_id in _ind_ids],
            key=lambda x: all_ind_map[x].fitness,
            reverse=True,
        )
    ...
    def _family_competition_selection_fn(
        ind_last_gen,
        ind_last_pop,
        n_parent,
        is_aggressive=is_aggressive,
        parent_size_threshold=parent_size_threshold,
    ):
        ...
        for ind in ind_last_gen:
            _selected_list, _unselected_list = _family_selection(
                ind, all_ind_map, parent_size_threshold
            )
        ...
        _candidates = sorted(
            candidate_set, key=lambda x: all_ind_map[x].fitness, reverse=True
        )
        _substitutes = sorted(
            substitute_set, key=lambda x: all_ind_map[x].fitness, reverse=True
        )
...
def diversity_awarness_selection_fn(
    next_inds: list[Individual],
    last_inds: list[Individual],
    n_parent: int,
    ...
    tuple_cands = sorted(
        list(zip(candidates, mean_similarity)), key=cmp_to_key(cmp_inds), reverse=True
    )
    _score_list = []
    _sim_list = []
    sorted_candidates = []
...
def max_divese_desc_get_parent_fn(
    individuals: list[Individual], n_parent: int, n_offspring: int
) -> list[PopulationQueryItem]:
...
def score_to_probability_with_logarithmic_curve(
    ...
    """
    if not all(
        isinstance(x, (int, float)) for x in [score, min_score, max_score, curve_factor]
    ):
    ...
    epsilon = 1e-10
    probability = math.log(normalized_score * curve_factor + epsilon + 1) / math.log(
        curve_factor + 1
    )
...
def test_diversity_awarness_selection_fn():
    ...
    selected = diversity_awarness_selection_fn(
        last_inds=last_inds, next_inds=next_inds, n_parent=2
    )
