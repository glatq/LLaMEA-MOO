...
def run_algo_eval_from_file_map(evaluator, file_map, options, is_baseline=False):
    ...
    for cls_name, code in _code_map.items():
        extra_init_params = {}
        if is_baseline:
            extra_init_params = baseline_algo_eval_param(
                evaluator.dim, evaluator.budget
            )
        ...
        if "save_dir" in options:
            ...
            file_path = os.path.join(
                save_dir, f"{cls_name}_{score:.4f}_{dim}D_{time_stamp}.pkl"
            )
...
if __name__ == "__main__":
    ...
    if is_plot:
        ...
        if use_mpi:
            ...
            with start_mpi_task_manager(
                result_recv_buffer_size=1024 * 1024 * 50
            ) as task_manager:
                if task_manager.is_master:
                    run_evaluation(
                        algo_name, algo_path, dim, save_dir, is_baseline=is_baseline
                    )
        else:
